...
DBusMessage *send_dbus_method_call(DBusConnection *conn, const char *dest, const char *path,
                                  const char *interface, const char *method) {
    DBusMessage *msg = dbus_message_new_method_call(dest, path, interface, method);
    if (!msg) {
...
dbus_bool_t get_property(DBusConnection *conn, const char *path, const char *interface,
                        const char *property, DBusMessageIter *iter) {
    DBusMessage *msg = send_dbus_method_call(conn, BLUEZ_SERVICE, path,
                                            "org.freedesktop.DBus.Properties", "Get");
    if (!msg) return FALSE;

    DBusMessageIter args;
    dbus_message_iter_init_append(msg, &args);
    if (!dbus_message_iter_append_basic(&args, DBUS_TYPE_STRING, &interface)) goto fail;
    if (!dbus_message_iter_append_basic(&args, DBUS_TYPE_STRING, &property)) goto fail;
...
dbus_bool_t set_property(DBusConnection *conn, const char *path, const char *interface,
    ...
    if (!dbus_message_iter_append_basic(&args, DBUS_TYPE_STRING, &interface)) goto fail;
    if (!dbus_message_iter_append_basic(&args, DBUS_TYPE_STRING, &property)) goto fail;
    if (!dbus_message_iter_open_container(&args, DBUS_TYPE_VARIANT, type == DBUS_TYPE_BOOLEAN ? "b" : "s", &variant)) goto fail;
    if (!dbus_message_iter_append_basic(&variant, type, value)) goto fail;
    if (!dbus_message_iter_close_container(&args, &variant)) goto fail;
...
int configure_adapter(DBusConnection *conn) {
    ...
    if (get_property(conn, ADAPTER_PATH, "org.bluez.Adapter1", "Powered", &iter)) {
        dbus_message_iter_get_basic(&iter, &value);
        if (!value) {
            printf("Powering on adapter...\n");
            value = TRUE;
            if (!set_property(conn, ADAPTER_PATH, "org.bluez.Adapter1", "Powered", DBUS_TYPE_BOOLEAN, &value)) {
                return -1;
            }
        }
    } else {
    ...
    if (get_property(conn, ADAPTER_PATH, "org.bluez.Adapter1", "Pairable", &iter)) {
        dbus_message_iter_get_basic(&iter, &value);
        if (!value) {
            printf("Setting adapter to pairable...\n");
            value = TRUE;
            if (!set_property(conn, ADAPTER_PATH, "org.bluez.Adapter1", "Pairable", DBUS_TYPE_BOOLEAN, &value)) {
                return -1;
            }
        }
    } else {
    ...
    if (get_property(conn, ADAPTER_PATH, "org.bluez.Adapter1", "Discoverable", &iter)) {
        dbus_message_iter_get_basic(&iter, &value);
        if (!value) {
            printf("Setting adapter to discoverable...\n");
            value = TRUE;
            if (!set_property(conn, ADAPTER_PATH, "org.bluez.Adapter1", "Discoverable", DBUS_TYPE_BOOLEAN, &value)) {
                return -1;
            }
        }
    } else {
...
DBusHandlerResult agent_message_handler(DBusConnection *conn, DBusMessage *msg, void *data) {
    const char *interface = dbus_message_get_interface(msg);
    const char *member = dbus_message_get_member(msg);

    if (strcmp(interface, "org.bluez.AgentManager1") != 0) return DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
...
int register_agent(DBusConnection *conn) {
    ...
    DBusMessage *msg = send_dbus_method_call(conn, BLUEZ_SERVICE, "/org/bluez",
                                            "org.bluez.AgentManager1", "RegisterAgent");
    if (!msg) {
        fprintf(stderr, "Error: Failed to create D-Bus message for RegisterAgent\n");
        return -1;
    }
    ...
    if (!dbus_message_iter_append_basic(&args, DBUS_TYPE_OBJECT_PATH, &agent_path)) {
    ...
    if (!msg) {
        fprintf(stderr, "Error: Failed to create D-Bus message for RequestDefaultAgent\n");
        return -1;
    }

    dbus_message_iter_init_append(msg, &args);
    if (!dbus_message_iter_append_basic(&args, DBUS_TYPE_OBJECT_PATH, &agent_path)) {
        fprintf(stderr, "Error: Failed to append AGENT_PATH to RequestDefaultAgent message\n");
        dbus_message_unref(msg);
        return -1;
    }
    ...
    dbus_bus_add_match(conn, "type='method_call',interface='org.bluez.Agent1'", &err);
    if (dbus_error_is_set(&err)) {
...
int pair_and_connect(DBusConnection *conn, const char *device_addr) {
    ...
    snprintf(device_path, sizeof(device_path), "/org/bluez/hci0/dev_%s", device_addr);
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; device_addr[i]; i++) {
        if (device_addr[i] == ':') device_addr[i] = '_';
    }
